...
echo ============================================================
echo   KnitOS — automated network test (CMD -^> WSL -^> QEMU)
echo ============================================================
echo.
...
echo     ^(guest: autotest fs^)
echo.
echo   Manual ^(two windows^):
echo     1. build.bat
echo     2. run-qemu-test.bat
echo     3. In QEMU: autotest fs
echo        then: autotest network 8080 16
...
echo     set MYOS_NIC=virtio     QEMU NIC ^(default; or rtl8139^)
echo.
echo   NIC regression:
echo     tests\auto-test.bat --rtl8139
echo     tests\auto-test-rtl8139.bat
echo.
...
if not defined HTTP_MAX_REQUESTS set "HTTP_MAX_REQUESTS=48"
...
echo [auto-test] NIC=!MYOS_NIC! CURL_HOST=!CURL_HOST! GUEST_IP=!GUEST_IP! HTTP_PORT=!HTTP_PORT! HTTP_MAX=!HTTP_MAX_REQUESTS!
echo.

wsl sed -i "s/\r$//" "!WSL_DIR!/tests/network/auto-network-test.sh" "!WSL_DIR!/tests/network/host-tests.sh" 2>nul

wsl /usr/bin/env MYOS_NIC=!MYOS_NIC! MYOS_VERBOSE=!MYOS_VERBOSE! MYOS_DIAG=!MYOS_DIAG! CURL_HOST=!CURL_HOST! GUEST_IP=!GUEST_IP! HTTP_PORT=!HTTP_PORT! HTTP_MAX_REQUESTS=!HTTP_MAX_REQUESTS! MYOS_HEADLESS=!MYOS_HEADLESS! /usr/bin/bash "!WSL_DIR!/tests/network/auto-network-test.sh" %*
set "RC=!errorlevel!"
